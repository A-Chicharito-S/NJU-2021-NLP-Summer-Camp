...
model = load_model('IAN_laptop_vocab4000_300_rms.hd5')
test_x_Context = test_x[:, :83, :]
test_x_Target = test_x[:, 83:, :]
...
with open(settings.LAPTOPS_TEST_WORDS, 'r', encoding='UTF=8') as r:
    with open(settings.LAPTOPS_TEST_PROCESSED, 'w', encoding='UTF-8') as w:
        lines = r.readlines()
        i = 0
        for line in lines:
            i = i + 1
